...
summary(m.adj) %>% coef() %>% as.data.frame %>% kable(format = 'latex')
...
mean(guesses.adj$EIG.optimal)
mean(guesses.adj$EIG.relative)

sd(guesses.adj$EIG)
sd(guesses.adj$EIG.optimal)
sd(guesses.adj$EIG.relative)

length(unique(guesses.adj$user))
length(unique(guesses.adj$gameId))
nrow(guesses.adj)
```
...
guesses.adj %>%
  select(EIG, EIG.optimal, EIG.random) %>%
