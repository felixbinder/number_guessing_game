...
guesses.bounds <- guesses %>%
  group_by(lowerBound, upperBound) %>%
  summarise(n = n(),
...
guesses.bounds %>%
  ggplot(aes(x = lowerBound, y = upperBound, fill = log10(n))) +
  geom_raster() + 
  theme_minimal() + 
  labs(
    title = "Heatmap of no. guesses by upper and lower bounds"
  )
...
guesses.bounds %>%
  ggplot(aes(x = lowerBound, y = upperBound, fill = EIG.relative)) +
  geom_raster() + 
  theme_minimal() + 
  scale_fill_distiller(palette="RdYlGn", direction = 1) +
  labs(
    title = "Heatmap of EIG relative by upper and lower bounds"
  )
...
guesses <- guesses %>%
  mutate(
    ...
    interval = numAvailable,
    lb.mod10.0 = lowerBound %% 10 == 0,
    lb.mod10.0.adj = (lowerBound - 1) %% 10 == 0,
...
We make a kitchen sink model with all of our measures. The measures are mostly added in order of 

```{r}

# Full model
m.all = lmer(EIG.relative ~ numAvailable + lowerBound + lb.mod10.0 + lb.lt.10 + sub.borrow +
...
# Full model
m.adj = lmer(EIG.relative ~ numAvailable + lowerBound + lb.mod10.0.adj + lb.lt.10 +
