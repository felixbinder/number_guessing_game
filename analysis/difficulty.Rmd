...
# Full model
m.adj = lmer(EIG.relative ~ interval + lowerBound.adj + carries + ops + (1 | user),
             data=guesses.adj, REML = F)
...
m.adj.slopes = lmer(EIG.relative ~ interval + lowerBound.adj + lb.mod10.0 + lb.lt.10 +
                            sub.borrow + sub.tens.eq + interval.odd + interval.tens.odd +
...
guesses.adj %>%
  ...
  scale_fill_distiller(palette="RdYlGn", direction = 1) +
  labs(
    title = "Heatmap of EIG relative by lower bound and interval"
  )
