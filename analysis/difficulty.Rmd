...
We extract the same measures under the assumption that 
...
guesses.adj <- guesses %>%
  mutate(
    ...
    # Adjusted lower bound for lazy updating
    # lowerBound.adj = case_when(
    #   lowerBound == 1 ~ 1,  # No adjustment when L = 1
    #   TRUE ~ lowerBound - 1  # If L has moved, decrement by 1 to simulate lazy updating
    # ),
    # Adjusted lower bound assuming user starts with 0
...
# Full model
m.adj = lmer(EIG.relative ~ interval + lowerBound + lb.mod10.0 + lb.lt.10 +
