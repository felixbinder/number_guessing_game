...
guesses$dataset = "Users"
guesses.binary$dataset = "Binary Search"
guesses.random$dataset = "Random"

all.guesses <-rbind(guesses.binary, guesses.random, guesses %>% select(-X, -user, -game_index, -no_user_games))

all.guesses %>%
  mutate(
    dataset = factor(dataset,
                     levels = c("Random", "Participants", "Users"))
    ) %>%
  filter(dataset != "Binary Search") %>%
  ggplot(aes(x = EIG.relative, color = dataset, fill=dataset)) +
  ...
  theme_minimal() +
  labs(
    title="Distribution of Relative EIG",
    x = "Relative EIG",
    y = "Density",
    fill="Dataset",
    color="Dataset"
...
## 1.2 How does EIG distribute across users?
...
users$dataset = "Users"
users.binary$dataset = "Binary Search"
...
all.users %>%
  mutate(
    dataset = factor(dataset,
                     levels = c("Random", "Users", "Binary Search"))
    ) %>%
  ggplot(aes(x = EIG, color=dataset, fill=dataset)) + 
