...
# Function to find entropy for a range

guessEntropy <- function(bounds) {

  # Get number of numbers available
  available = diff(bounds) + 1
  
  # Get prob of any number appearing
  prob = 1/available
  
  # Get entropy
  entropy = prob * log(prob) * available
  
  return(entropy)
  
}

# Update the bounds based on whether guess is higher or lower than targetNum

updateBounds <- function(bounds, guess, targetNum) {
...
# Define information gain of guess

infoGain <- function(bounds, guess, targetNum) {
  
  # Calculate entropy before
  entropy.prior = guessEntropy(bounds)
  
  # Update bounds based on guess
  bounds = updateBounds(bounds, guess, targetNum)
  
  # Calculate posterior entropy
  entropy.post = guessEntropy(bounds)
  
  # Calculate info gain
  gain = entropy.prior - entropy.post
  
  return (gain)
  
}

infoGain(c(1,100), 50, 49)
...
The optimal strategy is binary search (selecting the median number in the remaining range).

```{r}

# Implementation of binary search strategy

binarySearch <- function(targetNum) {
  guess = -1
  guess.history = c()
  bounds.history = c()
  lower = c()
  ...
  while(guess != targetNum) {
    ...
    bounds.history = c(bounds.history, bounds)
    
    # Make a guess (mean of lower and upr)
    guess = round(mean(bounds))
    
    # Get higher or lower and update bounds
    bounds = updateBounds(bounds, guess, targetNum)
    
    # Add to list of guesses
    guess.history = c(guess.history, guess)
  }
  
  df = data.frame(attemptNum = 1:length(guess.history),
                  guess = guess.history,
                  lowerBound = lower,
                  upperBound = upper)
...
guesses <- guesses %>%
  ...
  mutate(
    ...
    # info gain
    infoGain = entropy - lead(entropy),
    
    # Find optimal guess
    optimalGuess = floor((upperBound + lowerBound) / 2),
    
    # find no excluded
    numExcluded = numAvailable - lead(numAvailable),
...
propExcluded and infoGain are very highly correlated (~0.95), and I find propExcluded a lot easier to reason about, so I use that in the analyses below.

```{r}

guesses %>%
  filter(!is.na(infoGain)) %>%
  select(infoGain, propExcluded) %>%
  cor()

```


Get summary info for each game (the mean infoGain and propExcluded).
...
games.info = guesses %>%
  group_by(gameId) %>%
  summarise(
    mean.infoGain = mean(infoGain, na.rm = TRUE),
    mean.propExcluded = mean(propExcluded, na.rm = TRUE)
  ) %>%
  ungroup()

games.info %>%
  ggplot(aes(x = mean.propExcluded)) +
  geom_density()
...
# Merge ppt id onto guesses
guesses <- merge(guesses, games[,c("gameId", "user")])

guesses.by.user <- guesses %>%
  group_by(user) %>%
  summarise(
    propExcluded = mean(propExcluded, na.rm = TRUE),
    n_games = n_distinct(gameId),
    n_guesses = n(),
    propOptimal = mean(perfectGuess)
  ) %>%
  ungroup()

guesses.by.user %>%
  ggplot(aes(x = factor(n_games), y = propExcluded)) +
...
# Merge optoimality info into games
games <- merge(games, games.info[,c("gameId", "mean.propExcluded", "mean.infoGain")])

games %>%
  ggplot(aes(x = game_idx, y = mean.propExcluded)) +
  stat_summary(fun.data = mean_cl_boot, geom = "pointrange")


```

A linear model predicting mean proportion of numbers excluded across a game as a function of the user index of the game (how many games the player had played before that game) has a significant fit. However, the effect size is tiny and R^2 is < 0.001.

```{r}
model.excluded.idx = lm(mean.propExcluded ~ game_idx, data = games)

summary(model.excluded.idx)
...
guesses %>%
  filter(perfectGuess == 1) %>%
  select(user, gameId, targetNum, perfectGuess, lowerBound, upperBound, guess, optimalGuess, propExcluded)

```

Distribution of proportion of optimal guesses by user.

TODO: How much would we expect by chance?

```{r}

guesses.by.user %>%
  ggplot(aes(x = propOptimal, y = ..count..)) +
  geom_density()

```

There are only ~30 users who reliably use binary search (>90% of guesses are optimal, and user has made > 1 guess total).

```{r}

guesses.by.user %>%
  filter(propOptimal > 0.7, n_guesses > 1) %>%
  arrange(desc(n_guesses))
...
- find quantile of guess
  - deviation from optimal
    - higher or lower? depends on where we start?
    - anchoring?

- characterise other strategies

- fix propExcluded & infoGain for final guess
